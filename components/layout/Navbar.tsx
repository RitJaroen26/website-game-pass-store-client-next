...
//     // 1. ตั้งค่าเริ่มต้นให้เป็นค่า "ปลอดภัย" ก่อน
...
//         // เมื่อเปลี่ยน State คอมโพเนนต์จะรีเฟรชตัวเองทันที
//         setIsLoggedIn(false);
//         setUsername("Player");
//         setBalance("0.00");
//     };

//     // 3. ป้องกันการแสดงผลก่อนที่ข้อมูลจะถูกดึงมา
//     // วิธีนี้แก้ปัญหา UI กระพริบ และแก้ Error Hydration
//     if (!isMounted) {
//         return (
//             <header className="fixed top-0 w-full z-50 h-20 border-b border-white/10 bg-slate-950/80 backdrop-blur-xl shadow-[0_0_20px_rgba(0,242,255,0.1)]">
//                 {/* อาจจะใส่ Skeleton โหลดดิ้งตรงนี้ก็ได้ถ้าต้องการ */}
//             </header>
//         );
//     }
...
export default function Navbar() {
    // 1. กำหนด State เริ่มต้นแบบปลอดภัย (Server-Safe)
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            // 🌟 ทริคสำคัญ: บังคับให้ฟังก์ชันนี้ทำงานแบบ Asynchronous 
            // เพื่อหลีกเลี่ยง Error: "Calling setState synchronously..."
            await Promise.resolve();

            if (!isActive) return;

            // ตอนนี้ปลอดภัยแล้วที่จะเซ็ต State
            setIsClient(true);
    ...
    }, []); // ทำงานแค่ครั้งเดียวตอนโหลดเสร็จ
